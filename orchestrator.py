...
import time
import threading
from sensors import ShelfSensor
...
def monitor_shelf():
    """Background task that watches for human pickups without blocking the LLM."""
    
    while True:
        """Only check the sensor if Temi is actually out on a task"""
        if temi_busy and temi_current_task:
            if shelf_sensor.check_pickup():
                print("\n[SENSOR] Physical pickup detected. Automating task completion...")
                temi_task_done() 
        
        """Check every 0.5 seconds to keep the system responsive """
        time.sleep(0.5)
...
def temi_task_done():
    ...
    if not temi_busy or temi_current_task is None:
        return
